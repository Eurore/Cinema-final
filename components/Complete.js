const Complete = {
	...
	template: `
        <div class='complete'>
			...
			<div class="paypal_wrapper">
				<p>
					Имате възможност да платите Вашите билети още сега чрез <strong>PayPal</strong>,
					или в деня на прожекцията на място в киносалона:
				</p>
	...
	setup() {
		...
		const movieData = {
			...
			datetimeFull: cinema.movie.date,
			date: new Date(cinema.movie.date?.split(",")?.shift()).toLocaleDateString(),
			time: cinema.movie?.date?.split(",")?.pop().trim(),
